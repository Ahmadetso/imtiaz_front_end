...
onMounted(async () => {
  try {
    const response = await fetchInvoices() // Assuming fetchInvoices returns a promise
    invoices.value = response.data.data // Update the data structure
    console.log(invoices.value)
  } catch (error) {
    console.error('Error fetching invoices:', error) // Handle any errors
  }
})
</script>
